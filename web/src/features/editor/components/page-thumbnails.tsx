...
export function PageThumbnails() {
  ...
  if (!isOpen) {
    return (
      <div className="fixed left-0 top-14 h-[calc(100vh-3.5rem)] flex items-start bg-transparent pt-4 px-2 z-50 transition-all duration-300">
        <Button 
          variant="ghost" 
          size="icon" 
          className="h-8 w-8 rounded-full text-zinc-500 hover:text-zinc-900 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm shadow-sm border border-zinc-200 dark:border-zinc-800"
  ...
  return (
    <div className="fixed left-0 top-14 h-[calc(100vh-3.5rem)] w-64 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-950/80 flex flex-col z-50 transition-all duration-300 backdrop-blur-sm shadow-xl">
      <div className="h-14 flex items-center justify-between px-4 border-b border-zinc-200/50 dark:border-zinc-800/50 shrink-0">
        <div className="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 font-medium text-[13px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700">
        {thumbnails.map((thumb) => (
          ...
          >
            {/* The Thumbnail Container - A4 proportions (794x1123px) scaled to fit */}
            <div 
              ...
              style={{
                width: "224px",
                height: "317px", // 224 * 1.414 (A4 aspect ratio)
                transformOrigin: "top left",
              }}
            >
              {/* Content rendered at full A4 width (794px), then scaled down */}
              <div 
                className="absolute top-0 left-0 pointer-events-none overflow-hidden"
                style={{
                  width: "794px",
                  height: "1123px",
                  transform: "scale(0.282)", // 224/794
                  transformOrigin: "top left",
                }}
              >
                ...
                >
                  {/* Inner content wrapper with explicit width and aggressive text wrapping */}
                  <div 
