...
export const server = new Server({
  ...
  async onLoadDocument(data) {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      console.error(`[onLoadDocument] Error fetching doc ${documentName}:`, error.message);
    }
    ...
    if (docState?.ydoc_state) {
      try {
        ...
        console.error(`[onLoadDocument] Failed to parse existing state for ${documentName}. It may be in legacy HTML format. Ignoring...`);
      }
    }
  ...
  async onStoreDocument(data) {
    ...
    if (error) {
      console.error(`[onStoreDocument] Error saving doc ${documentName}:`, error.message);
    }
  },
});
